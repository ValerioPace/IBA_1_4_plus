<?php
	...
	$event = isset($event_id) ? Evento::withTrashed()->find($event_id) : null;
?>
...
{{ Form::open(array('url' => 'update_event', 'class' => 'form-horizontal', 'id' => 'edit_event', 'files' => true)) }}
	...
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-10">
		  	<button type="submit" name="restore" class="btn btn-primary" data-loading-text="Ripristino..." autocomplete="off">Ripristina</button>
		  	<button type="submit" name="permanent_delete" class="btn btn-default btn-danger" data-loading-text="Eliminazione..." autocomplete="off">ELIMINA</button>
...
<script type="text/javascript">
	...
	$('#edit_event').submit(function() {
		...
		if (button.attr('name') == 'permanent_delete'){
			if (!confirm("Eliminando questo evento non sarà più possibile ripristinarlo, continuare?")){
				button.button('reset');
				return false;
			}
		} else if (!confirm("Sei sicuro di voler ripristinare questo evento?")){
			button.button('reset');
			return false;
		}
		$.ajax({
			...
			success: function(response) {
				...
				if (button.attr('name') == 'permanent_delete'){
					$.ajax({
						url: 'console/user.active.events.expired.list',
						data: {page: 'expired_events'},
						type: 'post',
					...
					$('#expired_event_details').html(response);
